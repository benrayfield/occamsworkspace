...
//import mutable.game.mmgMouseAI.mouseRecorder.CommandlineMouseYRecorder;
import mutable.learnloop_partsMovedToMutableuitoollearnloop.old.ui.LearningVec_OLD;
...
*/
public class TimelessExperimentUtil{
	private TimelessExperimentUtil(){}
	...
	*/
	...
	static Map<String,float[][]> datasets = new HashMap();
	
	static synchronized float[][] dataset(String name){
		float[][] dataset = datasets.get(name);
		if(dataset != null) return dataset;
		if(name.equals("animalEmojiWallpaper70Dataset23x23")){
			dataset = loadAnimalEmojiWallpaper70Dataset(true);
		}else if(name.equals("animalEmojiWallpaper70Dataset23x23WithQuarterOfBitsOn")){
			dataset = normDataset(loadAnimalEmojiWallpaper70Dataset(true),fraction->(fraction>=.75)?1:0);
		}else if(name.equals("animalEmojiWallpaper70Dataset16x16WithQuarterOfBitsOn")){
			dataset = normDataset(loadAnimalEmojiWallpaper70Dataset(false),fraction->(fraction>=.75)?1:0);
		}else if(name.equals("animalEmojiWallpaper70Dataset23x23WithAllBrightnessEquallyOften")){
			dataset = normDataset(loadAnimalEmojiWallpaper70Dataset(true),fraction->fraction);
		}else if(name.equals("animalEmojiWallpaper70Dataset16x16WithAllBrightnessEquallyOften")){
			dataset = normDataset(loadAnimalEmojiWallpaper70Dataset(false),fraction->fraction);
		}else if(name.equals("animalEmojiWallpaper70Dataset16x16WithHalfOfBitsOn")){
			dataset = normDataset(loadAnimalEmojiWallpaper70Dataset(false),fraction->(fraction>=.5)?1:0);
		}else{
	...
	public static int trainingVecQuantity(String dataset){
		if(dataset.equals("animalEmojiWallpaper70Dataset23x23")){
			return dataset("animalEmojiWallpaper70Dataset23x23").length;
		}
		if(dataset.equals("animalEmojiWallpaper70Dataset23x23WithQuarterOfBitsOn")){
			return dataset("animalEmojiWallpaper70Dataset23x23WithQuarterOfBitsOn").length;
		}
		if(dataset.equals("animalEmojiWallpaper70Dataset16x16WithQuarterOfBitsOn")){
			return dataset("animalEmojiWallpaper70Dataset16x16WithQuarterOfBitsOn").length;
		}
		if(dataset.equals("animalEmojiWallpaper70Dataset23x23WithAllBrightnessEquallyOften")){
			return dataset("animalEmojiWallpaper70Dataset23x23WithAllBrightnessEquallyOften").length;
		}
		if(dataset.equals("animalEmojiWallpaper70Dataset16x16WithAllBrightnessEquallyOften")){
			return dataset("animalEmojiWallpaper70Dataset16x16WithAllBrightnessEquallyOften").length;
		}
		if(dataset.equals("animalEmojiWallpaper70Dataset16x16WithHalfOfBitsOn")){
			return dataset("animalEmojiWallpaper70Dataset16x16WithHalfOfBitsOn").length;
		}
	...
	public static float[] trainingVec(String dataset, int whichVec){
		if(dataset.equals("animalEmojiWallpaper70Dataset23x23")){
			return dataset("animalEmojiWallpaper70Dataset23x23")[whichVec];
		}
		if(dataset.equals("animalEmojiWallpaper70Dataset23x23WithQuarterOfBitsOn")){
			return dataset("animalEmojiWallpaper70Dataset23x23WithQuarterOfBitsOn")[whichVec];
		}
		if(dataset.equals("animalEmojiWallpaper70Dataset16x16WithQuarterOfBitsOn")){
			return dataset("animalEmojiWallpaper70Dataset16x16WithQuarterOfBitsOn")[whichVec];
		}
		if(dataset.equals("animalEmojiWallpaper70Dataset23x23WithAllBrightnessEquallyOften")){
			return dataset("animalEmojiWallpaper70Dataset23x23WithAllBrightnessEquallyOften")[whichVec];
		}
		if(dataset.equals("animalEmojiWallpaper70Dataset16x16WithAllBrightnessEquallyOften")){
			return dataset("animalEmojiWallpaper70Dataset16x16WithAllBrightnessEquallyOften")[whichVec];
		}
		if(dataset.equals("animalEmojiWallpaper70Dataset16x16WithHalfOfBitsOn")){
			return dataset("animalEmojiWallpaper70Dataset16x16WithHalfOfBitsOn")[whichVec];
		}
	...
	*/
	...
	static float[][] loadAnimalEmojiWallpaper70Dataset(boolean big){
		...
		byte[] picGrid;
		if(big) picGrid = Files.readFileOrInternalRel("/data/datasets/animal-emoji-wallpaper/Animal-Emoji-Wallpaper-70-with-Animal-Emoji-Wallpaper.scaledTo23x23.deduplicated.jpg");
		else picGrid = Files.readFileOrInternalRel("/data/datasets/animal-emoji-wallpaper/Animal-Emoji-Wallpaper-70-with-Animal-Emoji-Wallpaper.16x16Each.grayscale.removedDuplicates.jpg");
		try{
