...
import immutable.rbm.learnloop.WeightAndVelOfEdlay;
import immutable.rbm.learnloop.WeightAndVelOfRbm;
...
public class RBM implements Serializable{
	...
	*/
	public float[][][][] predict(float[][] vecs, float[][] attsMerged, boolean individualEdlaysAfterFullUp){
		return vecsZigzag(zigzagsLearn, attsMerged, vecs, weight, nolayIsScalar, individualEdlaysAfterFullUp, biasPerNodeside);
	}
	...
	*/
	public final boolean[] nolayIsScalar;
	
	//public final float learnRate;
	
	//neuralqlearnUsingScalarRbmAsLowFewLayersAndFeedforwardAboveThat
	public final boolean[] edlayIsFeedforward;
	...
	*/
	...
	public RBM(
		...
		boolean[] nolayIsScalar,
		//int lowNNolaysAreScalar, //FIXME this should be boolean[nolays] cuz of edlayIsFeedforward and neuralqlearning
		...
		this.nolayIsScalar = nolayIsScalar;
		...
		this.edlayIsFeedforward = edlayIsFeedforward;
	...
	public RBM(NavigableMap map){
		this(
			...
			JsonDS.floatArrayToBooleanArray((float[])map.get("nolayIsScalar")),
			...
			(boolean[]) JsonDS.floatArrayToBooleanArray((float[])map.get("edlayIsFeedforward")),
	...
	*/
	public NavigableMap asMap(){
		if(cachedAsMap == null){
			...
			m.put("nolayIsScalar", JsonDS.booleanArrayToFloatArray(nolayIsScalar));
			...
			m.put("edlayIsFeedforward", JsonDS.booleanArrayToFloatArray(edlayIsFeedforward));
	...
	*/
	...
	public static float[][] newZerosArraysSameSizeAsNodes(float[][][] edges){
	...
	*/
	public static float[][][][] vecsZigzag(int zigzags, float[][] attMerged, float[][] vecsVisibleNodes,
			float[][][] weight, boolean[] nolayIsScalar, boolean individualZigzagsAfterFullUp, float[][] biasPerNodeside){
		if(gpu){
			int nolays = weight.length+1;
			...
			if(individualZigzagsAfterFullUp){ //other kind of learning, never used for predicting visibleNodes
				...
				*/
				...
				for(int nolayOut=1; nolayOut<nolays; nolayOut++){ //inference from visibleNodes all the way up
					...
					boolean highNolayIsScalar = nolayIsScalar[nolayOut];
				...
				for(int z=1; z<zigzags; z+=2){
					...
					for(int nolayOut=nolays-2; nolayOut>=0; nolayOut--){ //each edlay down 1 individually
						...
						boolean lowNolayIsScalar = nolayIsScalar[nolayOut];
			...
			for(int v=0; v<vecs; v++){
				float[][][] oldZZ = zigzag(zigzags, attMerged, vecsVisibleNodes[v], weight, nolayIsScalar, Rand.strongRand, biasPerNodeside);
				if(oldZZ == null){
					throw new Error("WTF");
				}
				for(int z=0; z<zigzags; z++){
	...
	*/
	public static float[][][] zigzag(int zigzags, float[][] Att, float[] visibleNodes,
			float[][][] weight, boolean[] nolayIsScalar, Random rand, float[][] biasPerNodeside){
		int nolays = weight.length+1;
		float[][][] ret = new float[zigzags][nolays][];
		ret[0][0] = visibleNodes; //its ok to return this cuz arrays must be used as immutable
		for(int z=0; z<zigzags; z+=2){
			...
			for(int nolayOut=1; nolayOut<nolays; nolayOut++){ //inference from visibleNodes all the way up
				float[] highNodesBias = biasPerNodeside[nolayOut*2+1];
				//boolean nolayOutIsScalar = nolayOut<lowNNolaysAreScalar;
				boolean nolayOutIsScalar = nolayIsScalar[nolayOut];
				ret[z][nolayOut] =
