...
public class WindowsJdkCompiler implements JavaCompiler{
	...
	public String classpathString(){
		...
		File tempLibDir = new File(Files.dataDir,"tempLib");
	...
	public Class compile(String... classParts){
		...
		if(debuggable() && !pausesSoPersonCanPutBreakpoint()){
			System.err.println("WARNING: You may not be able to debug in runtime generated java code since !pausesSoPersonCanPutBreakpoint(), which in Eclipse for example requires you refresh the dir the .java file is generated in then view that file, then click OK to the popup caused by pausesSoPersonCanPutBreakpoint (and may also require putting a breakpoint in it, TODO experiment more with that). See JavaCompiler.java for details.");
		}
		...
		*/
		...
		if(classFile.exists()){
			if(javaFile.exists()){ //both exist
				String javaFileContentExisting = Text.bytesToStr(Files.read(javaFile));
	...
	public static String findCompilerOnHardDrive(int maxFoldersToSearch){
		...
		//FIXME (2019-2-10 experiment after github)
		//String p = "c:\\java\\openjdk-11.0.2_windows-x64_bin\\"; //FIXME dont write paths that only exist on my computer
		String p = "C:\\java\\openjdk-9.0.4_windows-x64_bin\\"; //FIXME dont write paths that only exist on my computer
		//String p = "c:\\java\\jdk1.8.0_151\\"; //FIXME dont write paths that only exist on my computer
		if(new File(p+"bin\\"+javacExeFileName).exists()){
			return p;
		}
