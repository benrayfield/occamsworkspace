...
import mutable.learnloop_partsMovedToMutableuitoollearnloop.old.RbmDisplay;
import mutable.learnloop_partsMovedToMutableuitoollearnloop.old.RbmPanel;
...
public class PaintSlidingVecUi extends Slidinglearnrandvecui{
	...
	*/
	...
	protected double lastTimeGameControllerInput = Time.now();
	...
	public PaintSlidingVecUi(Var<RBM> rbmVar, Var<RbmDisplay> varDisplayWhat, Var<RBM> prototype, int vecSize,
		...
		for(int y=0; y<paintGrid.length; y++){
			for(int x=0; x<paintGrid[0].length; x++){
				paintGrid[y][x] = Rand.weightedCoinFlip(.2) ? 1 : 0;
			}
		}
	...
	/*FIXME moving this to paramsVar
	...
	*/
	public float[] nextTrainingVecOrCounterexample(boolean isExample, boolean isRandom){
		...
		int datasetSize = TimelessExperimentUtil.trainingVecQuantity(datasetName);
		...
		float[] vec = TimelessExperimentUtil.trainingVec(datasetName, whichVecInThatDataset); //FIXME can return duplicate vector if dataset is too small
	...
	protected void onGameControllerInput(){
		...
		if(buttons.contains(KeyEvent.VK_R)){
			...
			paintGrid[selectedY][selectedX] = Rand.weightedCoinFlip(avePixelBright())?1:0;
		...
		lastTimeGameControllerInput = Time.now();
	...
	static boolean mainFuncsWindowExitsJavaOnClose = true;
	
	public static void mainWithoutWindowExitingOnClose(){
		mainFuncsWindowExitsJavaOnClose = false;
		main(new String[0]);
	}

	public static void main(String[] args){
		JFrame window = new JFrame("HumanAiNet - RBM editor - opensource MIT license");
		if(mainFuncsWindowExitsJavaOnClose){
			window.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
			mainFuncsWindowExitsJavaOnClose = true;
		}
		...
		for(int z=0; z<zigzagPredictArray.length; z++) zigzagPredictArray[z] = RBM.newZerosArraysSameSizeAsNodes(rbmEdges);
		...
		for(int z=0; z<zigzagNormArray.length; z++) zigzagNormArray[z] = RBM.newZerosArraysSameSizeAsNodes(rbmEdges);
		...
		*/
		...
		//TODO put all the params here, such as datasetName (or hash of it? or of its generator?)
		//and batchSize, and things that would go in Slide* (but skipping Slide and going to binufnode/occamsfuncer after this)
		Var<NavigableMap> paramsVar = new Var(JsonDS.emptyMap);
		...
		TimelessExperimentUtil.rbmVar = rbmVar;
		TimelessExperimentUtil.learningVecs = slidingBars.learningVecs;
		...
		controlsPanel.add(new JScrollPane(slidingBars, ScrollPaneConstants.VERTICAL_SCROLLBAR_NEVER, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED));
		boolean bootLispBeforePersonPushesEnter = false;
		controlsPanel.add(new LearnFuncRbmEditor(rbmVar, paramsVar));
		...
		outerPanel.add(controlsPanel,BorderLayout.NORTH);
		...
		while(window.isVisible()){
			double gameControllerInputHowLongAgo = Time.now()-slidingBars.lastTimeGameControllerInput;
