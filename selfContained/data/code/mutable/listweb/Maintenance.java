	...
	*/
	public static synchronized void loadAndSaveAllVarFilesToMakeExistenceInPrilistSymmetric(){
		...
		try{
			...
			ListwebRoot.loadAllFiles();
			SortedSet<String> namesInNonsymmetricPairs = new TreeSet();
			long countChanges = 0;
			for(String name : ListwebRoot.listweb.keySet()){
				lg("Checking "+name);
				for(String name2 : (List<String>)ListwebRoot.prilist(name)){ //name.prilist contains name2
					if(!ListwebRoot.prilist(name2).contains(name)){ //name2.prilist not contains name
						countChanges++;
						namesInNonsymmetricPairs.add(name);
						namesInNonsymmetricPairs.add(name2);
						lg("Nonsymmetric prilist, add "+name+" to "+name2+"'s prilist");
						ListwebRoot.addToEndOfPrilistIfNotExist(name2, name);
					}
				}
			}
