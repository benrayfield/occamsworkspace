...
public class EmbeddedReberGrammar extends DataSet {
	...
	public EmbeddedReberGrammar(Lazycl lz, Random r) throws Exception {
		this.lz = lz;
		...
		training = generateSequences(lz, r, total_sequences);
		validation = generateSequences(lz, r, total_sequences);
		testing = generateSequences(lz, r, total_sequences);
	}
	...
	public static List<DataSequence> generateSequences(Lazycl lz, Random r, int sequences) {
		...
		for (int sequence = 0; sequence < sequences; sequence++) {
			...
			while (true) {
				...
				steps.add(new DataStep(lz, observation, target_output));
			}
