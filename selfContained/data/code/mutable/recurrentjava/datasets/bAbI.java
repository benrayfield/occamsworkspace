...
import mutable.recurrentjava.loss.LossArgMax;
...
public class bAbI extends DataSet{
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("Lazycl = "+lz);
		
		Random rng = new Random();
		bAbI data = new bAbI(lz, 3, 100, true, rng);
		
		System.out.println("done.");
	}
	
	public bAbI(Lazycl lz, int setId, int totalExamples, boolean onlySupportingFacts, Random rng) throws Exception {
		this.lz = lz;
		...
		lossReporting = new LossArgMax(lz);
			...
			for (Statement statement : story.statements) {
				if (statement.isFact) {
					for (int w = 0; w < statement.fact.size(); w++) {
						...
						steps.add(new DataStep(lz, input, null));
					}
				}
				else {
					for (int w = 0; w < statement.question.size(); w++) {
						...
						steps.add(new DataStep(lz, input, targetOutput));
					}
					float[] input = new float[inputDimension];
